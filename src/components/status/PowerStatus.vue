      ...
      <div v-else class="p-3 space-y-2.5">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="text-lg font-bold">🔋 {{ power.batteryLevel.toFixed(0) }}%</span>
            <span class="text-[10px] text-muted-foreground font-mono">
              {{ power.isCharging ? power.adapterName : $t('status.on_battery') }}
            </span>
          </div>
          <CommonTooltip :content="wattageTooltip" as-child>
            <span class="text-lg font-bold font-mono tabular-nums cursor-default">
        ...
        <div class="flex items-center justify-between text-xs text-muted-foreground">
          ...
          <span v-else>—</span>
          <span v-if="untilTime" class="font-mono">until {{ untilTime }}</span>
        </div>

        <!-- Row 4: Power breakdown (local only) -->
        <div v-if="!power.isRemote" class="flex items-center gap-3 text-[10px] text-muted-foreground font-mono">
          <span>Screen <span class="font-semibold text-foreground">{{ (power.brightnessPower || 0).toFixed(1) }}w</span></span>
          <span>·</span>
          <span>SoC <span class="font-semibold text-foreground">{{ (power.heatpipePower || 0).toFixed(1) }}w</span></span>
          <span v-if="power.isCharging">·</span>
          <span v-if="power.isCharging">Loss <span class="font-semibold text-foreground">{{ (power.efficiencyLoss || 0).toFixed(0) }}mw</span></span>
        </div>
      </div>
    </template>
